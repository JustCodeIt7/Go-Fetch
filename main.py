...
def render_sidebar() -> CrawlerSettings:
    ...
    max_pages = st.sidebar.slider("Max Pages to Crawl", 5, 300, 5)
...
def render_download_buttons():
    ...
    st.download_button(
        "Download All as Markdown", combined_md, "site.md", "text/markdown"
    )
    ...
    st.download_button(
        "Download ZIP of Pages", buf.getvalue(), "pages.zip", "application/zip"
    )
...
def main():
    ...
    st.markdown(
        "**Note**: `pip install crawl4ai` & run `crawl4ai-setup` before first use."
    )
